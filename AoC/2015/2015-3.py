...
STRATEGY
    Part 1: Build a dict keyed by [int, int].
        Use 'in' keyword to test whether dict already has key.
        Length of dict is how many distinct keys it has.
"""
...
def main():
    ...
    map_d = {}
    pos = (0, 0)
    map_d[pos] = 1  # initial present delivery

    # PART 1: How many unique houses did Santa deliver to?
    for c in list(s):
        # adjust the position
        if c == '>':  # increase x
            pos = (pos[X] + 1, pos[Y])
    ...
    print("length of dict is:", len(map_d))  # 2592 is correct



# Way too many ['^', 'v', '>', '<'] to easily fit here, so I just
# jammed the whole thing on one line. NB: .strip() it!
DATA = """\
