from tkinter import *

class Dummy: pass
var = Dummy()

root = Tk()
root.option_readfile('optionDB')
root.title('Checkbutton')
for castmember, row, col, status in [
    ('John Cleese', 0,0,NORMAL), ('Eric Idle', 0,1,NORMAL),
    ('Graham Chapman', 1,0,DISABLED), ('Terry Jones', 1,1,NORMAL),
    ('Michael Palin',2,0,NORMAL), ('Terry Gilliam', 2,1,NORMAL)]:
    setattr(var, castmember, IntVar())
    cb = Checkbutton(root, text=castmember, state=status, anchor=W, variable = getattr(var, castmember))
    cb.grid(row=row, column=col, sticky='w')  # NB: 'col' is an ambiguous keyword here: update to: 'column'

print(dir(var))

root.mainloop()

