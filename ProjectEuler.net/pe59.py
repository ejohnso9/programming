...
def letterCount(loi: list[int], key_i: int) -> int:
    count = 0
    for _i in loi:
        i = _i ^ key_i
        c = chr(i)
        count += int(isLetter(c))
...
def main(ints: list[int]):
    ...
    _max = 0
    atoz_i = range(ord('a'), ord('z') + 1)
    for c_i in atoz_i:
        c = chr(c_i)
        lc = letterCount(_c, c_i)
        if lc > _max:
            _max = lc
        print(f"'{c}' count is: {lc}")

    print(f"_max is: {_max}")  # jsg

    # now, decode the whole thing
    _idx = 0
    key_i = [ord(c) for c in 'exp']
    letters = []
    numbers = []
    for _i in ints:
        i = _i ^ key_i[_idx]
    ...
    print(''.join(letters))
    print(f"sum is: {sum(numbers)}")  # 129448 accepted on 2024Nov12
    ...
    for key_i in atoz:
        ints = [key_i ^ _i for _i in _c]
        counts_d = freq_dict(ints)
        tup_ls = sorted([(count, k) for k, count in counts_d.items()], reverse=True)
        s = ''.join([chr(t[1]) for t in tup_ls[:10]])
        # print(key_i, f"'{s}'")
        print(key_i, chr(key_i), f"'{s}'")
